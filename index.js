...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6289509938750@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["6289509938750@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["6289509938750@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			case 'owner':
                ...
                case 'tahta':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/hartatahta?text=${teks}&apikey=H527Y93838H2OUQR27`)
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'ttp':
					...
					anu = await fetchJson(`https://mhankbarbar.tech/api/text2image?text=${ttp}&apiKey=SLpvUgOcMYwIx0pFeELt`, {method: 'get'})
				...
				case 'cerpen':
					gatauda = body.slice(7)
					anu = await fetchJson(`https://arugaz.my.id/api/cerpen`, {method: 'get'})
               ...
               case 'infogempa':
                ...
                case 'resepmasakan':
                   ...
                   break
                case 'indohot':
                   anu = await fetchJson(`https://arugaz.my.id/api/indohot`, {method: 'get'})
                   if (anu.error) return reply(anu.error)
				...
				case 'ytsearch':
					...
					anu = await fetchJson(`https://mhankbarbar.tech/api/ytsearch?q=${body.slice(10)}&apiKey=Nkwp9oR7Eg9yykbejBpm`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = '=================\n'
				...
				case 'tiktok':
					...
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbar.tech/api/tiktok?url=${args[0]}&apiKey=Nkwp9oR7Eg9yykbejBpm`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
			    ...
			    case 'igstalk':
					...
					ige = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/igprofile?query=${ige}&apikey=H527Y93838H2OUQR27`, {method: 'get'})
				...
				case 'infomobil':
					...
					ige = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/infomobil?merk=${ige}&apikey=H527Y93838H2OUQR27`, {method: 'get'})
				...
				case 'persengay':
					if (args.length < 1) return reply('Apa yang mau dicari um?')
					anu = await fetchJson(`https://arugaz.my.id/api/howgay`, {method: 'get'})
				...
				break
					...
					break
				case 'say':
					anu = await fetchJson(`https://arugaz.my.id/api/bapakfont?kata=${body.slice(6)}`, {method: 'get'})
					reply('nianjim\n\n'+anu.result)
					break
				case 'artinama':
					if (args.length < 1) return reply('Apa yang mau dicari um?')
				...
				case 'simi':
					if (args.length < 1) return reply('Textnya mana um?')
					teks = body.slice(5)
					anu = await simih(teks) //fetchJson(`https://st4rz.herokuapp.com/api/simsimi?kata=${teks}`, {method: 'get'})
